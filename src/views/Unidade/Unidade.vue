...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/Unidade/Unidadealterar.vue'),
        Alterar:   () => import('@/views/Unidade/Unidadecadastrar.vue'),
    },
    name: 'Cliente',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Unidade', value: 'unidade', class: "cyan dark 1 white--text" },
            ...
            nomeformulario: 'Unidade'
        }
    },
    methods: {
        carregaForm() {
            this.unidade.visualiza = true
            EventBus.$emit('carregacadastro', this.unidade)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadosunidade = process.env.VUE_APP_HOST + "unidade/search/" + key

            this.axios.get(urldadosunidade)
            .then(response => {           
        ...
        alteraritem(item) {
            this.unidade.id = item.id
            this.unidade.unidade = item.unidade
            this.unidade.visualiza = true
            EventBus.$emit('carregaalteracao', this.unidade)
            this.initialize()
        },
    },
