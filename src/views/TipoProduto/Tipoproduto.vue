...
export default {
    components: {   
        ...
        Cadastrar: () => import('@/views/TipoProduto/Tipoprodutocadastrar.vue'),
        Alterar:   () => import('@/views/TipoProduto/Tipoprodutoalterar.vue'),
    },
    name: 'TipoProduto',
    data() {
        return {
            ...
            headers: [{
                ...
                { text: 'Tipo Produto', value: 'tipoproduto', class: "cyan dark 1 white--text" },
    ...
    methods: {
        carregaForm() {
            this.tipoproduto.visualiza = true
            EventBus.$emit('carregacadastro', this.tipoproduto)
            this.initialize()
        },
        initialize() {
            const  key = 'frota2021house'
            const  urldadostipoprodutoe = process.env.VUE_APP_HOST + "tipoproduto/search/" + key

            this.axios.get(urldadostipoprodutoe)
            .then(response => {           
        ...
        alteraritem(item) {
            this.tipoproduto.id = item.id
            this.tipoproduto.tipoproduto = item.tipoproduto
            this.tipoproduto.visualiza = true
            EventBus.$emit('carregaalteracao', this.tipoproduto)
            this.initialize()
        },
    },
