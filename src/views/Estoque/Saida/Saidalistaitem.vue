<template>
    <div>
        <v-dialog  v-model="dialog" height="900px" max-width="950px" persistent :retain-focus="false">
            ...
            <v-card height="600px" width="950px" rounded>
...
export default {
    ...
    data() {
        return {
            ...
            headersitem: [{
                ...
                { text: 'saidaid', value: 'saida', align: ' d-none',  class: "cyan dark 1 white--text" },
                { text: 'Ação', align: 'center', value: 'actions', sortable: false, class: "cyan dark 1 white--text" }
            ],
            lintaitens: [],   
            overlay: false,   
            formulario: 'Lista de Itens da Saida',
    ...
    methods: {
        exibeitens(item) {
            const  urldadossaidaitem = process.env.VUE_APP_HOST + "saidaitem/list/" + item
            this.overlay = true
            this.itensprojeto = item.id
            this.axios.get(urldadossaidaitem)
            .then(response => {           
    ...
    mounted(){ // gerencia o receber de dados de outro componente
        EventBus.$on('carregalista', (saida) => {
            localStorage.ensaidaid = saida.id
            this.exibeitens(saida.id)
            this.dialog = true
            this.alert  = true
        })
    },   
